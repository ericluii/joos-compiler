...
Test_A5::Test_A5(std::string& stdlibPath, std::string& directoryPath) :
    directoryPath(directoryPath),
    stdlibPath(stdlibPath) {}

void Test_A5::init() {
    test_name = "Static Analysis.";
    test_description = "This tests encapsualtes all A5 required tests.";
}
...
void Test_A5::test() {
    ...
    for(unsigned int i = 0; i < a5TestFiles.size(); i++) {
        Error::resetErrors();
        ...
        for (unsigned int j = 0; j < stdlibFilesA5.size(); j++) {
            files.push_back(stdlibPath + "/" + stdlibFilesA5[j]);
        }

        for (unsigned int j = 0; j < a5TestFiles[i].size(); j++) {
            files.push_back(directoryPath + "/" + a5TestFiles[i][j]); 
        }
        ...
        if (a5TestFiles[i][0][1] == 'e') {
            checkTrue("A5: " + a5TestFiles[i][0], Error::count() != 0,
